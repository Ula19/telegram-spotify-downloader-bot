...
Порядок цепочки (выстроен по данным из прода — живые и быстрые сверху):

1. `spotify-downloader-v2` — RapidAPI, 320kbps (метадату добирает через Spotify oEmbed)
2. `spotify-music-mp3-downloader-api` — RapidAPI, богатая метадата
3. `spotify-downloader-mp33` — RapidAPI
4. `spotify-downloader23` — RapidAPI
5. `spotify-downloader9` — RapidAPI (часто таймаутит — держим внизу как запас)
6. `spotdl` — self-hosted (качает с YouTube через yt-dlp), последний рубеж, когда легли все RapidAPI. Ходит на YouTube через WARP-прокси (`SPOTDL_PROXY_URL`), т.к. YouTube часто блокирует прямые/датацентр-IP
...
- **Cloudflare WARP** (socks5) — прокси для spotdl, обход блокировки YouTube
- **Docker Compose** — бот + postgres + warp + autoheal
